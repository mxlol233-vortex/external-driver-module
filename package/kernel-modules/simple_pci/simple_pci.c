 ...
 **/
#define BAR 0
#define CDEV_NAME "simple_pci"
#define SIMPLE_PCI_DEVICE_ID 0x2333
...
static struct pci_device_id pci_ids[] = {
	...
	{ PCI_DEVICE(QEMU_VENDOR_ID, SIMPLE_PCI_DEVICE_ID), },
	{ 0, }
};
...
static ssize_t read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
	ssize_t ret;
	u32 kbuf;
        printk(KERN_INFO "SIMPLE_PCI: read() len: %lx offset: %lx\n",len, *off);
	if (*off % 4 || len == 0) {
		ret = 0;
	} else {
...
static ssize_t write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
	ssize_t ret;
	u32 kbuf;
        printk(KERN_INFO "SIMPLE_PCI: write()\n");
...
static int myinit(void)
{
	...
	if (pci_register_driver(&pci_driver) < 0) {
		printk(KERN_INFO "SIMPLE_PCI error registering the custom_qemu_device_driver\n");
		return 1;
	}
	printk(KERN_INFO "SIMPLE_PCI success on registering the custom_qemu_device_driver\n");
	return 0;
}
